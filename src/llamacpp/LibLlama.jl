...
# The library path is a settable global (ccall re-resolves it per call, and
# the dylib's @loader_path rpath finds its ggml dependencies beside it).
# Empty until resolved — Wink.local_model! fills it from WINK_LIBLLAMA or the
# vendored-release default; regenerate this file with spike/generate_bindings.jl
# after a vendor bump (this prologue lives in Wink's copy only).
libllama::String = ""
set_lib!(path::AbstractString) = (global libllama = String(path); nothing)
