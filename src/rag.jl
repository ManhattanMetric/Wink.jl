...
function build_index!(; io::IO = CONFIG.status_io)
    isempty(CONFIG.embed_model) &&
        throw(WinkResolveError("no embedding provider configured (set OPENAI_API_KEY, " *
                               "run a local OpenAI-compatible server, or " *
...
tool_search_docs(query::String, top_k::String) = tool_guard("search_docs") do
    ...
    isempty(CONFIG.embed_model) &&
        return keyword_fallback(q,
            "no embedding provider (set OPENAI_API_KEY, run a local " *
            "OpenAI-compatible server, or " *
            "configure!(embed_model = ..., embed_api_base = ...))")
    idx = ensure_index!()
    idx === nothing &&
