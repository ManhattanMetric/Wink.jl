...
Show a proposed action (`kind` is `:eval`, `:edit`, `:write`, `:shell`, or
`:abstract` — an abstraction proposed during context compaction) and ask the
...
function _connect_hint(e)
    ...
    host = try
        String(HTTP.URI(e.url).host)
    catch
        return nothing
    end
    v6first = try
        first(Sockets.getalladdrinfo(host)) isa Sockets.IPv6
    catch
        false
    end
